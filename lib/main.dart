    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        textTheme: const TextTheme(
          headlineMedium: TextStyle(fontSize: 42, fontWeight: FontWeight.w700, height: 1.1),
              ...
              children: [
                Container(
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        Text(_formattedNow(), style: Theme.of(context).textTheme.labelLarge),
                        const Spacer(),
                        Container(
                          ...
                          child: const Row(
                            children: [
                              Icon(Icons.circle, size: 10, color: Color(0xFF00CFA0)),
                              SizedBox(width: 8),
                              Text('Healthy', style: TextStyle(fontWeight: FontWeight.w600)),
                            ],
                          ),
                        ),
                        ...
                        const CircleAvatar(
                          ...
                          child: Text('OP', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                        ),
                      ],
                    ),
                  ),
          ...
          children: [
            ...
            const Padding(
              ...
              child: Column(
                ...
                children: [
                  Text('System v2.4.1', style: TextStyle(color: Color(0xFF6B7DA8), fontSize: 12)),
                  SizedBox(height: 6),
                  Text('All systems operational', style: TextStyle(color: Color(0xFF5A6BFF), fontSize: 12)),
                ],
              ),
            ),
          ],
          ...
          decoration: BoxDecoration(
            ...
            gradient: selected
                ? const LinearGradient(
                    colors: [Color(0xFF1A284B), Color(0xFF0E1A38)],
                  )
                : null,
            boxShadow: selected
                ? const [
                    BoxShadow(
                      color: Color(0x404868FF),
                      blurRadius: 14,
                      spreadRadius: -4,
                    ),
                  ]
          ...
          child: Row(
            children: [
              Icon(
                ...
                size: 20,
                color: selected ? const Color(0xFF6B7BFF) : const Color(0xFF8C9BBC),
              ),
              const SizedBox(width: 12),
              Text(
                item.title,
                style: TextStyle(
                  color: selected ? const Color(0xFF7A89FF) : const Color(0xFF94A2C3),
                  fontSize: 16,
      ...
      drawer: Drawer(
        child: SafeArea(
          child: ListView(
            children: [
              const ListTile(
                title: Text('Aero Arc', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700)),
              ),
...
void _navigateTo(BuildContext context, AppSection next) {
  if (ModalRoute.of(context)?.settings.name == next.route) {
    return;
  }
...
Widget _sectionPage(AppSection section) {
  return switch (section) {
    ...
    AppSection.relays => const RelaysPage(),
    AppSection.agents => const AgentsPage(),
    AppSection.registry => const RegistryPage(),
    AppSection.nodes => const NodesPage(),
    AppSection.telemetry => const TelemetryPage(),
    AppSection.events => const EventsPage(),
...
enum AppSection {
  ...
  static AppSection fromLocation(String? location) {
    ...
    final path = '/${uri?.pathSegments.firstOrNull ?? 'overview'}';
